...
"مره واحد اسمو عفرتو  كان بيرجم ابليس ف الحج قالولو ليه؟ قال عشان يمكن احتاجو 👻😹",
...
async def alive(client: Client, message):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("𝙶𝚁𝙾𝚄𝙿 ", url=f"{gr}"),
            ...
            [ 
                 InlineKeyboardButton("اضف البوت الي مجموعتك ❤️", url="https://t.me/{client.me.username}?startgroup=true")
            ]
        ]
    )

    alive = f"""╭──── • ◈ • ────╮
么 [ᯓ 𝚂𝙾𝚞𝚁𝙲𝙴 𝙰𝙵𝚁𝙾𝚃𝙾𝙾 ](t.me/UI_VM)  .
么  [𝙳𝙴𝚅  𝙰𝙵𝚁𝙾𝚃𝙾𝙾](t.me/VVYVVJ)  .
╰──── • ◈ • ────╯
...
async def alive(client: Client, message):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("العربية 🇪🇬", callback_data="arbic"),
...
@Client.on_message(filters.command(["عفرتو","مطور السورس","عفرتو باشا"], ""))
...
async def set_vi_so(client: Client, message):
   NAME = await client.ask(message.chat.id,"**♪ ارسل رابط لوجو السورس  💎 .\n♪ مثال ⟨ https://telegra.ph/file/04b2f1f1c808dc49db35b.jpg ⟩  💎 .**", filters=filters.text, timeout=30)
   VID_SO = NAME.text
   bot_username = client.me.username
...
async def llink(client: Client, message: Message):
    if not message.from_user.username in ["VVYVVJ"]:
      return
