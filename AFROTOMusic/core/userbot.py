...
class Userbot(Client):
    ...
    async def start(self):
        ...
        if config.STRING1:
            ...
            try:
                await self.one.join_chat("UI_VM")
                await self.one.join_chat("T_Y_E_X")
            except:
                pass
            assistants.append(1)
            try:
                await self.one.send_message(config.BOT_ID, "» تم تشغيـل الحسـاب المسـاعـد .. بنجـاح ✅")
            except:
        ...
        if config.STRING3:
            ...
            LOGGER("ميــوزك عفرتو").info(f"Assistant Three Started as {self.three.name}")
        ...
        if config.STRING4:
            ...
            LOGGER("ميــوزك عفرتو").info(f"Assistant Four Started as {self.four.name}")
        ...
        if config.STRING5:
            ...
            LOGGER("ميــوزك عفرتو").info(f"Assistant Five Started as {self.five.name}")

    async def stop(self):
        LOGGER("ميــوزك عفرتو").info(f"Stopping Assistants...")
        try:
            if config.STRING1:
                await self.one.stop()
            if config.STRING2:
